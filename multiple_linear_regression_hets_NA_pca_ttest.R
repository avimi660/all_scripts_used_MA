...
for (i in 2:(dim(transposed_sites_het_match)[2]-40)) {
  for (j in (i+1):(dim(transposed_sites_het_match)[2]-39)) {
    ...
    if (dim(temp_cols)[1]>1) {
      if (!(sd(temp_cols[,1])==0 | sd(temp_cols[,2])==0)) {
...
#[1,] "chr1_11363882" "prop_mismatch" "0.877130273601014"  "TRUE"
#[2,] "chr1_11363882" "mtDNA_clade"   "-0.880352146983195" "TRUE"
#[3,] "prop_mismatch" "mtDNA_clade"   "-0.994825246628158" "TRUE"

filtered_transposed_sites_het_match <- filtered_transposed_sites_het_match %>% select(-chr1_11363882,-prop_mismatch)
